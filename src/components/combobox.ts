...
export class ComboBox extends Component implements IComboBox {
  ...
  constructor(config: IComboBoxConfig) {
    ...
    this.#listBoxItemClassNames_ = union(config.listBoxItemClassNames || [], [
      `${this.name}${DEFAULT_SEPERATOR}listbox${DEFAULT_SEPERATOR}item`,
    ])
    ...
    this.classNames = union(config.wrapperClassNames || [], this.classNames, [
      this.name,
      `${this.name}${DEFAULT_SEPERATOR}list`,
    ])
  }
